    ...
    let type_map = List.fold_left (fun map (p, e) ->
      ...
      CoqName.Map.fold (fun var typ1 map ->
          ...
          if PathName.Set.mem path rejected_vars then
            ...
            let map' = Type.unify env typ1 typ2 in
            Name.Map.union (fun _ typ1 typ2 ->
                Some (Type.unify_monad env typ1 typ2))
              map map')
...
let rec effects (env : Type.t FullEnvi.t) (e : (Loc.t * Type.t) t)
  ...
  | Variable ((l, typ), x) ->
    begin try
      ...
      let vars_map = Type.unify env typ' typ in
      let typ' = Effect.map_type_vars vars_map typ' in
      let typ = Type.unify_monad ~collapse:false env typ typ' in
      Variable ((l, typ), x)
    with Not_found ->
  ...
  | Tuple ((l, typ), es) ->
    ...
    let typ = Type.unify_monad env typ @@ Effect.join d @@ Type.Tuple typs in
    Tuple ((l, typ), es)
  ...
  | LetVar ((l, typ), x, e1, e2) ->
    ...
    let effect = Type.unify_monad env typ @@ Effect.join descriptor typ2 in
  ...
  | Match ((l, typ), e, cases) ->
    ...
    let (d, typ_cases) = Effect.split @@
      Type.union env (List.map (fun (_, e) -> snd (annotation e)) cases) in
    let descriptor = Effect.Descriptor.union [d_e; d] in
    let typ = Type.unify_monad env typ @@ Effect.join descriptor typ_cases in
    Match ((l, typ), e, cases)
  ...
  | IfThenElse ((l, typ), e1, e2, e3) ->
    ...
    let typ = Type.unify_monad env typ @@ Effect.join d @@
      Type.unify_monad env typ2 typ3 in
    IfThenElse ((l, typ), e1, e2, e3)
  ...
  | Coerce ((l, _), e, typ) ->
    let e = effects env e in
    let typ = Type.unify_monad env typ @@ snd @@ annotation e in
    Coerce ((l, typ), e, typ)
  | Sequence ((l, typ), e1, e2) ->
    let (d, e1, e2, typ2) = match compound [e1; e2] with
      | (d, [e1; e2], [_; typ2]) -> (d, e1, e2, typ2)
      | _ -> failwith "Wrong answer from 'compound'." in
    let typ = Type.unify_monad env typ @@ Effect.join d typ2 in
    Sequence ((l, typ), e1, e2)
  | Run ((l, typ), x, d, e) ->
    let e = effects env e in
    let (d_e, typ_e) = Effect.split @@ snd @@ annotation e in
...
and effects_of_def_step (env : Type.t FullEnvi.t)
  ...
  { def with Definition.cases =
    def.Definition.cases |> List.map (fun (header, e) ->
      ...
      let e = effects env e in
      let typ = Type.unify_monad env header.Header.typ @@ snd @@ annotation e in
...
let rec monadise (env : unit FullEnvi.t) (e : (Loc.t * Type.t) t) : Loc.t t =
  ...
  | For ((l, _), name, down, e1, e2, e) ->
    monadise_list env [e1; e2] d [] (fun es' ->
      ...
      | _ -> failwith "Wrong answer from 'monadise_list'.")
  | Coerce ((l, d), e, typ) ->
    Coerce (l, monadise env e, Type.unify_monad env d typ)
